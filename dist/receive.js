...
var amqp = __webpack_require__(0);

amqp.connect('amqp://127.0.0.1', function (err, connection) {
    connection.createChannel(function (err, chanel) {
        var queue = 'test_query';

        chanel.assertQueue(queue, { durable: false });
        console.log('RabbitMQ logger: Waiting for messages in ' + queue + '. \n                    To exit press CTRL+C');

        chanel.consume(queue, function (msg) {
            var parseMsg = msg.content.toString();
            console.log(msg);
